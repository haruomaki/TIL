...
歪度/尖度とは、**正規化した確率変数のn次キュムラント**のこと。
...
\begin{aligned}
  \varphi_Y(t) &= \mathbb{E}[e^{itY}] \\
            &= \mathbb{E}[e^{\frac{itX}{\sigma} - \frac{i\mu t}{\sigma}}] \\
            &= e^{-i(\mu / \sigma)t} \varphi_X \left( \frac{t}{\sigma} \right) \\
\end{aligned}
```

```math
\tilde{\kappa}_n = \kappa_n^{(Y)} = \frac{\kappa_n^{(X)}}{\sigma^n}
\hspace{1em} (n \ge 2)
```

これを標準化キュムラントと呼ぶことにする。具体的には

```math
\begin{aligned}
  \tilde{\kappa}_{1} &= 0 \\
  \tilde{\kappa}_{2} &= 1 \\
  \tilde{\kappa}_{3} &= \frac{\mu_{3}}{\sigma^3} \\
  \tilde{\kappa}_{4} &= \frac{\mu_{4}}{\sigma^4} - 3 \\
  \tilde{\kappa}_{5} &= \frac{\mu_{5}}{\sigma^5} - 10 \frac{\mu_{3}}{\sigma^3} \\
  \tilde{\kappa}_{6} &= \frac{\mu_{6}}{\sigma^6} - 10 \frac{\mu_{3}^{2}}{\sigma^6} - 15 \frac{\mu_{4}}{\sigma^4} + 30 \\
\end{aligned}
```
